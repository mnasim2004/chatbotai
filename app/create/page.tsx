...
const Home= ()=> {
  ...
  useEffect(()=>{
    ...
    const [form, setForm] = useState<{
        ...
        files: File[]; // Explicitly set files as File[]
        description: string;
    ...
    const submitform = async ()=>{
        ...
        const response = await axios.post("http://127.0.0.1:8000/create-chatbot", formData, {
            headers: {
                "Content-Type":"multipart/form-data"
            }
        })
        ...
        router.push("/all")
        toast({
            title: "Successfully created Bot",
                                ...
                                <FileUpload onFileChange={
                                    (event: React.ChangeEvent<HTMLInputElement>) => {
                                        ...
                                        if (fileList) {
                                          ...
                                          setForm({
                                            ...form,
                                            files: Array.from(fileList), // Convert FileList to File[]
                                          });
                                        }
                                }}/>
                                {form.files.length > 0 && (
                                    <table className="rounded-corners" style={{ marginTop: '20px', width: '30%'}}>
                                    <thead>
                                        <tr>
                                        <th>Filename</th>
                                        </tr>
